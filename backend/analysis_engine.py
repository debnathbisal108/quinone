...
CLASSIFICATION_RESPONSE_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "has_readable_nutrition_information": {"type": "boolean"},
        "has_readable_ingredients": {"type": "boolean"},
        "confidence": {"type": "number"},
        "evidence": {"type": "string"},
    },
    "required": [
        "has_readable_nutrition_information",
...
# Label retries use a shorter contract. The first pass still requests every
# supported field, while this contract lets Gemini omit unprinted/null
# nutrients instead of spending output tokens serializing dozens of nulls.
# `_normalize_label_result` restores the stable application-facing shape.
_COMPACT_LABEL_NUTRIENT_SCHEMA = {
    "type": "object",
    "properties": _LABEL_NUTRIENT_PROPERTIES,
}
COMPACT_LABEL_RESPONSE_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "brand": _NULLABLE_STRING_SCHEMA,
        "product_name": {"type": "string"},
    ...
    "required": [
        "product_name", "nutrition_per_serving", "nutrition_per_100g",
        "ingredients", "ocr_confidence",
    ],
}
...
# =============================================================================
label_prompt = """
You are Nutrica's Packaged Food Analyzer. The supplied images are views of the
SAME package: the first is the user's complete photograph and any later images
are enlarged detail crops generated by the backend. Combine readable evidence
across those views; never treat the crops as separate products.

Extract the printed nutrition table and ingredients. Return ONLY the JSON
required by the supplied response schema.
...
- Do not copy manufacturer addresses, customer-care text, preparation
  directions, legal text, or marketing paragraphs into claims.
- Keep brand, product name, ingredients, allergens and claims concise. Product
  name means the food/product variant, not all text visible on the package.
...
# =============================================================================
classify_prompt = """
You are checking whether a user has ALREADY supplied usable packaged-food
...
def _label_detail_views(image: Image.Image) -> list[Image.Image]:
    """Keep the full package while making small label text independently legible.

    Users should never need to crop a package manually. The first view preserves
    context; four overlapping quadrants enlarge panels printed on only one part
    of a pouch, box, bottle, or wrapper.
    """
    ...
    x_mid = width // 2
    y_mid = height // 2
    overlap_x = max(30, round(width * 0.1))
    overlap_y = max(30, round(height * 0.1))
    boxes = [
    ...
    for attempt in range(1, GEMINI_JSON_MAX_ATTEMPTS + 1):
        ...
        if attempt > 1:
            if (
                ...
                config.get("response_json_schema")
                is LABEL_RESPONSE_JSON_SCHEMA
                or config.get("response_json_schema")
                is COMPACT_LABEL_RESPONSE_JSON_SCHEMA
            ):
                retry_instruction = (
                    "These are a complete package photo and enlarged crops of "
                    "that SAME package. Extract only the product name, printed "
                    "nutrition numbers, and printed ingredient names. Never "
                    "copy addresses, legal text, directions, or marketing "
                    "paragraphs. Omit unprinted nutrient keys; do not calculate "
                    "or invent values. Return only the compact schema JSON."
                )
                retry_config["response_json_schema"] = (
                    COMPACT_LABEL_RESPONSE_JSON_SCHEMA
                )
        ...
        try:
            ...
            last_error = error
            logger.warning(
                "Gemini %s returned no readable response text on attempt "
                "%d/%d (elapsed_ms=%.1f): %s",
                operation,
                attempt,
                GEMINI_JSON_MAX_ATTEMPTS,
...
def _validate_model_contract(
    ...
    if (
        schema is LABEL_RESPONSE_JSON_SCHEMA
        or schema is COMPACT_LABEL_RESPONSE_JSON_SCHEMA
    ):
        ...
        has_ingredients = (
            ...
            and any(str(item or "").strip() for item in ingredients)
        )
        if not nutrition_values and not has_ingredients:
            raise ValueError(
...
def classify_image(client, image):
    ...
    A classifier failure must never be converted to ``food``. That old fallback
    created a false back-label request for the exact label photo the user had
    already supplied.
    """
    last_error: Exception | None = None
    ...
    raise ModelJSONResponseError(
        "The AI could not determine whether the image contains a nutrition "
        "label. Please retry the same image."
    ) from last_error


def _normalize_label_result(label: dict[str, Any]) -> dict[str, Any]:
    """Return one stable label shape without inventing unprinted values."""
    normalized = dict(label)
    normalized["brand"] = normalized.get("brand") or None
    normalized["product_name"] = str(
        normalized.get("product_name") or "Packaged Food"
    ).strip()
    ...
    normalized["servings_per_container"] = normalized.get(
    ...
    for key in ("nutrition_per_serving", "nutrition_per_100g"):
        ...
        if not isinstance(supplied, dict):
            supplied = {}
        normalized[key] = {
            nutrient: supplied.get(nutrient)
    ...
    confidence = normalized.get("ocr_confidence")
    try:
        normalized["ocr_confidence"] = max(0.0, min(1.0, float(confidence)))
...
def analyze_meal(
    ...
    # Initial uploads may be either meal photographs OR nutrition back-labels.
    # Image type must be established before meal extraction; otherwise printed
    # ingredients/artwork on a label can be misread as a branded meal and the
    # backend asks for the label that the user already supplied.
